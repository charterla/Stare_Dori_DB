...
class Database():
    ...
    def createTableForEvent(self, event_id: int, server_id: Optional[int] = 2):
        cursor = self.connection.cursor()
        table = f"CREATE TABLE IF NOT EXISTS \"{server_id}_{event_id}_event_players_test\" ("\
              ...
              + f");"
        index = f"CREATE INDEX IF NOT EXISTS \"{server_id}_{event_id}_event_players_nowPoints_desc_test\" "\
              + f"ON \"{server_id}_{event_id}_event_players_test\" (nowPoints DESC NULLS LAST);"
        cursor.execute(table)
        cursor.execute(index)
        table = f"CREATE TABLE IF NOT EXISTS \"{server_id}_{event_id}_event_points_test\" ("\
              + f" time BIGINT,"\
              + f" uid INTEGER,"\
              + f" value INTEGER,"\
              + f" PRIMARY KEY (uid, value),"\
              + f" CONSTRAINT to_player FOREIGN KEY (uid) REFERENCES \"{server_id}_{event_id}_event_players_test\"(uid)"\
              + f");"
        cursor.execute(table)
        table = f"CREATE TABLE IF NOT EXISTS \"{server_id}_{event_id}_event_intervals_test\" ("\
              + f" uid INTEGER,"\
              + f" startTime BIGINT,"\
              + f" endTime BIGINT,"\
              ...
              + f");"
        cursor.execute(table)
        table = f"CREATE TABLE IF NOT EXISTS \"{server_id}_{event_id}_event_rank_test\" ("\
              + f" uid INTEGER,"\
              + f" updateTime BIGINT,"\
              + f" fromRank SMALLINT,"\
              + f" toRank SMALLINT,"\
              ...
              + f" CONSTRAINT to_player FOREIGN KEY (uid) REFERENCES \"{server_id}_{event_id}_event_players_test\"(uid)"\
              + f");"
        index = f"CREATE INDEX IF NOT EXISTS \"{server_id}_{event_id}_event_rank_updateTime_desc_test\" "\
              + f"ON \"{server_id}_{event_id}_event_rank_test\" (updateTime DESC NULLS LAST);"
        cursor.execute(table)
        cursor.execute(index)
        function = f"CREATE OR REPLACE FUNCTION \"{server_id}_{event_id}_event_newPoints_test\"() RETURNS TRIGGER AS $$\n"\
                 + f"BEGIN\n"\
                 + f"    UPDATE \"{server_id}_{event_id}_event_players_test\" SET nowPoints = new.value, lastUpdateTime = new.time\n"\
                 + f"        WHERE uid = new.uid AND nowPoints < new.value;\n"\
                 + f"    RETURN NEW;\n"\
                 + f"END;\n"\
                 + f"\n"\
                 + f"$$ LANGUAGE plpgsql;"
        trigger = f"CREATE OR REPLACE TRIGGER \"{server_id}_{event_id}_event_newPoints_test\" "\
                + f"AFTER INSERT ON \"{server_id}_{event_id}_event_points_test\" "\
                + f"FOR EACH ROW EXECUTE PROCEDURE\"{server_id}_{event_id}_event_newPoints_test\"();"
        cursor.execute(function)
        cursor.execute(trigger)
        function = f"CREATE OR REPLACE FUNCTION \"{server_id}_{event_id}_event_newUpdate_test\"() RETURNS TRIGGER AS $$\n"\
                 + f"DECLARE \n"\
                 + f"    checking RECORD;\n"\
                 + f"    now RECORD;\n"\
                 + f"BEGIN\n"\
                 ...
                 + f"            FROM \"{server_id}_{event_id}_event_players_test\") \n"\
                 + f"    LOOP \n"\
                 + f"        FOR now IN (SELECT * FROM \"{server_id}_{event_id}_event_rank_test\" \n"\
                 + f"            WHERE uid = checking.uid ORDER BY updateTime DESC LIMIT 1) \n"\
                 + f"        LOOP \n"\
                 + f"            IF now.toRank <> checking.rank THEN \n"\
                 + f"                INSERT INTO \"{server_id}_{event_id}_event_rank_test\" (uid, updateTime, fromRank, toRank) \n"\
                 ...
                 + f"        INSERT INTO \"{server_id}_{event_id}_event_intervals_test\" (uid, startTime, endTime, valueDelta) \n"\
                 ...
                 + f"\n"\
                 + f"$$ LANGUAGE plpgsql;"
        trigger = f"CREATE OR REPLACE TRIGGER \"{server_id}_{event_id}_event_newUpdate_test\" "\
                + f"AFTER UPDATE OF nowPoints ON \"{server_id}_{event_id}_event_players_test\" "\
                + f"FOR EACH ROW EXECUTE PROCEDURE\"{server_id}_{event_id}_event_newUpdate_test\"();"
        cursor.execute(function)
        cursor.execute(trigger)
        self.connection.commit()
        cursor.close()
    ...
    def insertEventPlayers(self, event_id: int, players: list[dict], default_time: int, server_id: Optional[int] = 2):
        ...
        players_value = [f"({player['uid']}, \'{player['name']}\', \'{player['introduction']}\', {player['rank']}, 0, {default_time})"
                         for player in players]
        self.insertOrUpdateData(
            f"INSERT INTO \"{server_id}_{event_id}_event_players_test\" (uid, name, introduction, rank, nowPoints, lastUpdateTime) "\
    ...
    def insertEventPoints(self, event_id: int, points: list[dict], server_id: Optional[int] = 2):
        ...
        self.insertOrUpdateData(
            f"INSERT INTO \"{server_id}_{event_id}_event_points_test\" (time, uid, value) "\
    ...
    def getEventPlayerName(self, event_id: int, uid: int, server_id: Optional[int] = 2):
        result = self.getData(f"SELECT name FROM \"{server_id}_{event_id}_event_players_test\" WHERE uid = {uid};")
    ...
    def getEventTopPlayers(self, event_id: int, ranking: Optional[int] = 10, server_id: Optional[int] = 2):
        return self.getData(
            f"SELECT * FROM \"{server_id}_{event_id}_event_players_test\" ORDER BY nowPoints DESC LIMIT {ranking};")
        
    def getEventPlayerPointsAtTimeBefore(self, event_id: int, uid: int, time_before: Optional[int] = 3600000, server_id: Optional[int] = 2):
        result = self.getData(
            f"SELECT value FROM \"{server_id}_{event_id}_event_points_test\" "\
    ...
    def getEventPlayerRecentPoints(self, event_id: int, uid: int, num: Optional[int] = 41, server_id: Optional[int] = 2):
        return self.getData(
            f"SELECT time, value FROM \"{server_id}_{event_id}_event_points_test\" "\
    ...
    def getEventPlayerPointsNumAtTimeAfter(self, event_id: int, uid: int, time_after: Optional[int] = 3600000, server_id: Optional[int] = 2):
        return self.getData(
            f"SELECT COUNT(uid) FROM \"{server_id}_{event_id}_event_points_test\" "\
    ...
    def getEventPlayerIntervals(self, event_id: int, uid: int, server_id: Optional[int] = 2):
        return self.getData(
            f"SELECT startTime, endTime, valueDelta FROM \"{server_id}_{event_id}_event_intervals_test\" "\
    ...
    def getEventPlayerRanks(self, event_id: int, uid: int, num: Optional[int] = None, server_id: Optional[int] = 2):
        return self.getData(
            f"SELECT updateTime, fromRank, toRank FROM \"{server_id}_{event_id}_event_rank_test\" "\
    ...
    def getEventPlayerRankUpToTopTimes(self, event_id: int, uid: int, server_id: Optional[int] = 2):
        return self.getData(
            f"SELECT updateTime FROM \"{server_id}_{event_id}_event_rank_test\" "\
    ...
    def getEventPlayersRankChangesAtTimeAfter(self, event_id: int, time_after: Optional[int] = 60000, server_id: Optional[int] = 2):
        return self.getData(
            f"SELECT uid, fromRank, toRank FROM \"{server_id}_{event_id}_event_rank_test\" "\
